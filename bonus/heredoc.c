...
void handle_cmd_path_hd(t_pipex *pipex, int i, t_node **gc, char **cmd_args, char *outfile)
{
...
void setup_hd(t_pipex *pipex, char *av[], char *env[], t_node **gc)
{
    ...
    pipex->n_cmds = 2;
    pipex->env = env;
    pipex->cmds = gc_malloc(gc, (sizeof(char *) * 2));
...
void handle_here_doc(int ac, char *av[], char *env[], t_node **gc)
{
    ...
    {
        pipex.pids[i] = fork();
        if (pipex.pids[i] == 0) // child
            execute_cmd_hd(&pipex, i, gc, av[5]);
        else if (pipex.pids[i] == -1)
            (gc_clear(gc), perror("fork"), exit(1));
    }
    
    (close(pipex.pipes[0][0]), close(pipex.pipes[0][1]), close(pipex.pipes[1][1]), close(pipex.pipes[1][0]));
    (waitpid(pipex.pids[0], &status, 0), waitpid(pipex.pids[1], &status, 0));
    pipex.status = WEXITSTATUS(status);
    (gc_clear(gc), exit(pipex.status));
}
